...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [marker_data_aligned,affine_xform] = realignMarkerSpacetimeDLC(cds,marker_data,varargin)
...
for i=1:length(cds.analog)
    ...
    if any(strcmp(header,'SyncPulse'))
        pulse_idx = i;
        break
    end
end
if pulse_idx > 0
    kinect_times = realignMarkerTimesDLC(cds.analog{pulse_idx},marker_data,true);
else
...
if ~rotation_known
    ...
    % find alignment
    plot_flag=1;
    [ affine_xform, kinect_pos ] = get_affine_xformDLC( cds, kinect_times, marker_data, x_lim_handle, y_lim_handle, plot_flag );
    %Save a file w/ affine_xform, so it can be used for other files from the
    %same day
    
end
% affine_xform = affine_xform * [1 0 0 0; 0 cosd(-45) -sind(-45) 0; 0 sind(-45) cosd(-45) 0; 0 0 0 1];

% [ kinect_pos ] = do_affine_xformDLC( marker_data, affine_xform, plot_flag);
